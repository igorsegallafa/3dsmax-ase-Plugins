...
void AsciiExp::DumpPoint3Keys(Control* cont, int indentLevel) 
{
	...
	if (ikc && cont->ClassID() == Class_ID(TCBINTERP_POINT3_CLASS_ID, 0)) {
		...
		for (i=0; i<ikc->GetNumKeys(); i++) {
			...
			fwprintf(pStream,L"\t%s\t%s\t%s\t%s\t%s\n", Format(key.tens).data(), Format(key.cont).data(), Format(key.bias).data(), Format(key.easeIn).data(), Format(key.easeOut).data() );
		}
		fwprintf(pStream,L"%s\t\t}\n", indent.data());
	}
	// Bezier point3
		...
		for (i=0; i<ikc->GetNumKeys(); i++) {
			...
			fwprintf(pStream,L"%s\t\t\t%s %d\t%s",
				...
				key.time, 
				Format(key.val).data() );
			fwprintf(pStream,L"\t%s\t%s\t%d\n", Format(key.intan).data(), Format(key.outtan).data(), key.flags);
		}
		fwprintf(pStream,L"%s\t\t}\n", indent.data());
	}
	// Bezier color
