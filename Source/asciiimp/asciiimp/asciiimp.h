...
public:
	...
	BOOL	Compare(const TCHAR* token, const TCHAR* id);
	TCHAR*	GetToken();
	BOOL	SkipBlock();
	Point3	GetPoint3();
	int		GetInt();
	...
	BOOL	GetFaceList(TriObject* tri, Matrix3& tm);
	BOOL	GetTVertexList(TriObject* tri);
	BOOL	GetTFaceList(TriObject* tri, Matrix3& tm);

	//Mapping Channel import support by Sandurr
	BOOL	GetTVertexListMap(TriObject* tri, int mp);
	BOOL	GetTFaceListMap(TriObject* tri, int mp, Matrix3& tm);
	BOOL	GetMappingChannel(TriObject* tri, Matrix3& tm);

	BOOL	GetCVertexList(TriObject* tri);
	BOOL	GetCFaceList(TriObject* tri, Matrix3& tm);
	BOOL	GetMeshNormals(TriObject* tri);
	BOOL	GetMesh(TriObject* tri, Matrix3& tm);
	...
	// Physique
	BOOL		  ImportPhysique(INode *pNode);
	void		  FinalizePhysique();
